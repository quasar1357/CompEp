...
### ### ### Define, run and plot the SIR model
...
run_sir <- function(s2_rate = 0, b = 1.5, g = 1, t = seq(0, 70, by = 0.01)) {
  
  ## Initial proportion in each compartment
  ## s2_rate = initial mount of people that only get immune after 2 infections
  i_tot_zero <- 1e-6
  i2_zero <- i_tot_zero * s2_rate # Note: as long as i_tot_zero is small, the rate of i1_zero to i2_zero has no big impact
...
plot_sir <- function(SIR_df, detailed = FALSE, pdf = "") {
  ...
  ## Make linear data frame
  ## With option detailed = TRUE, S1/I1 and S2/I2 are treated separately; if FALSE, the totals of S and I are used instead
  if (detailed){
...
compare_s2_rates <- function(s2_rates, beta, gamma, t = seq(0, 70, by = 0.01), csv = FALSE){
  ...
  ## With the option csv = TRUE, save the data in a csv file
...
## compare different betas in one plot
...
##compare different betas in separate plots
...
## Create SIR plots (temporal evolution) for chosen betas and S2_rates, both with and without detailed depiction of S1 and S2
